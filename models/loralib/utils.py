...
def apply_lora(args, clip_model):
    ...
    if args['encoder'] == 'vision' or args['encoder'] == 'both':
        # print(INDEX_POSITIONS_VISION[args['backbone']])
...
def save_lora(args, list_lora_layers):
    ...
    save_dir = args['output'] + '/checkpoints/' + args['model']
    os.makedirs(save_dir, exist_ok=True)

    save_path = f'{save_dir}/' + args['loss'] + str(args['num_train_category']) + '_best_ACC_lora.pt'
...
def load_lora(args, list_lora_layers):
    # to manage names like ViT-B/16
    # backbone = args.backbone.replace('/', '').replace('-', '').lower()
    load_path = args['output'] + '/checkpoints/' + args['model'] + '/' + args['loss']+str(args['num_train_category'])+'_best_ACC_lora.pt'
